...
public class WaveFileReader {
    ...
    public WaveFileReader(Context context,String filename) {

        this.initReader(context,filename);
    }
    ...
    private void initReader(Context context, String filename) {
        this.filename = filename;

        try {
            fis = context.getResources().getAssets().open(this.filename);
            ...
            if (samplerate != 16000) {
                throw new Exception("sample rate wrong!!!");
            }
            this.byterate = readLong();
            this.blockalign = readInt();
            this.bitspersample = readInt();

            if (subchunk1size == 18){//过滤字节为18时 这里多两个字节附加数据
                readBytes(2);
            }
            ...
            for (int i = 0; i < this.len; ++i) {
                for (int n = 0; n < this.numchannels; ++n) {
                    ...
                    } else if (this.bitspersample == 16) {//是这个
